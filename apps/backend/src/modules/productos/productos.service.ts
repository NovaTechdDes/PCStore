...
import { ActualizarProductoDTO, CrearProductoDTO, FiltrosProductoDTO } from "./productos.schema";
...
// 1. Calcular precio
export async function calcularPrecio(costo: number, costoDolar: number, iva: number, ganancia: number) {
  let base: number;
...
export const listarProductos = async (filtros: FiltrosProductoDTO) => {
    ...
    const result = await request.query(`
        SELECT p.*, m.Nombre AS MarcaNombre, pr.Nombre AS ProveedorNombre, u.Nombre AS UnidadNombre
        FROM Productos p
        JOIN Marcas m ON m.Id = p.MarcaId
        JOIN Proveedores pr ON pr.Id = p.ProveedorId
        JOIN UnidadesMedida u ON u.id = p.UnidadId
        ${where}
...
export const obtenerProductoPorId = async(id: Number) => {
    ...
    const caracteristicasResult = await pool.request()
    .input("id", sql.Int, id)
    .query("SELECT Id, Clave, Valor FROM ProductoCaracteristicas WHERE ProductoId = @id");
    
    const imagenesResult = await pool.request()
    .input('id', sql.Int, id)
    .query("SELECT Id, RutaArchivo, EsPrincipal FROM ProductoImagenes WHERE ProductoId = @id");
...
export const crearProducto = async(data: CrearProductoDTO, archivos: Express.Multer.File[]) => {
    const pool = await getPool();
    ...
    try {
        ...
        const precio = await calcularPrecio(data.costo, data.costoDolar, data.iva, data.ganancia);

        //Verificar codigo interno unico
        const existe = await new sql.Request(transaction).input('codigoInterno', sql.NVarChar(50), data.codigoInterno)
        .input('descripcion', sql.NVarChar(255), data.descripcion)
        .query('SELECT Id From Productos WHERE CodigoInterno = @codigoInterno OR Descripcion = @descripcion');

        if(existe.recordset.length > 0){
            throw {status: 409, msg: "Ya existe un producto con ese codigo interno o descripcion"}
        }
        ...
        for(const c of data.caracteristicas){
            await new sql.Request(transaction)
                ...
                .input('valor', sql.NVarChar(150), c.valor)
                .query(`
...
export const actualizarProducto = async(id: number, data: ActualizarProductoDTO) => {
  const pool = await getPool();
...
export async function agregarImagenes(id: number, archivos: Express.Multer.File[]) {
  const pool = await getPool();
  ...
  const tieneImagenPrincipal = producto.imagenes.some((img: any) => img.EsPrincipal);
...
export async function eliminarImagen(imagenId: number) {
  ...
  const rutaFisica = path.join(process.cwd(), imagen.RutaArchivo.replace(/^\//, ""));
