...
export const MarcaItem = ({ item }: Props) => {
  ...
  return (
    <tr key={item.Id} className="hover:bg-slate-50 dark:hover:bg-zinc-800/60 transition-colors duration-150 border-b border-slate-200 dark:border-zinc-800/60 text-sm">
      {/* Código / ID */}
      <td className="py-3.5 px-4 font-mono text-xs font-semibold text-amber-600 dark:text-amber-400 whitespace-nowrap">#{item.Id.toString().padStart(2, '0')}</td>
      ...
      {/* Descripción */}
      <td className="py-3.5 px-4 text-slate-600 dark:text-zinc-400 max-w-xs truncate text-xs">{item.Descripcion}</td>

      {/* Sitio Web */}
      <td className="py-3.5 px-4 text-xs whitespace-nowrap">
        {item.SitioWeb ? (
          <a href={item.SitioWeb} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-blue-600 dark:text-blue-400 hover:underline">
      ...
      <td className="py-3.5 px-4 text-right whitespace-nowrap">
        <div className="flex items-center justify-end gap-1.5">
          ...
          <button
            disabled={isPending}
            onClick={deleteMarca}
            className={`p-1.5 text-slate-400 ${isPending ? 'text-slate-400 cursor-not-allowed' : 'hover:text-red-600 dark:text-zinc-500 dark:hover:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10'} transition-all cursor-pointer`}
