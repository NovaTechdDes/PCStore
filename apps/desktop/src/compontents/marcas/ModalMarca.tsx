      ...
      <div className="bg-white dark:bg-[#18181b] border border-slate-200 dark:border-zinc-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4.5 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between bg-slate-50/60 dark:bg-zinc-900/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-slate-900 dark:text-zinc-100">{marca ? 'Editar Marca' : 'Nueva Marca'}</h2>
              <p className="text-xs text-slate-500 dark:text-zinc-400">{marca ? `Modificando información de ${marca.Nombre}` : 'Completa los campos para registrar la marca comercial'}</p>
            </div>
          </div>
          <button
            onClick={cerrarModal}
        ...
        <form onSubmit={(e) => e.preventDefault()} className="p-6 space-y-4 overflow-y-auto">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-zinc-300 mb-1.5 uppercase tracking-wide">Descripción u Observaciones</label>
            <textarea
              rows={3}
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-zinc-300 mb-2 uppercase tracking-wide">Estado en el Catálogo</label>
